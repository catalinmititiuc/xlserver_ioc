...
class RequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if form.list[0].filename.split('.')[0] + ".html" in files:
            fi = open(form.list[0].filename.split('.')[0] + ".html", 'r')
       	    result = fi.read().decode('utf-8')
        else:
            result = upload_table(form.list[0].file, form.list[0].filename)

            fi = open(form.list[0].filename.split('.')[0] + ".html", 'w')
            fi.write(result.encode('utf-8'))
            fi.close()
